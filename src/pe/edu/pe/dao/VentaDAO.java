...
import pe.edu.pe.entidad.DetalleVenta;
import pe.edu.pe.entidad.Venta;
import java.sql.Statement;
import pe.edu.pe.daoImpl.interfaceCrudVenta;
...
public class VentaDAO implements interfaceCrudVenta<Venta, DetalleVenta> {
    ...
    public List<Venta> consultaFechas(Date fechaInicio, Date fechaFin) {
        List<Venta> registros = new ArrayList();
        try {
            ps = cx.conectar().prepareStatement("SELECT v.id,v.usuario_id,u.nombre as usuario_nombre,"
                    + "v.persona_id,p.nombre as persona_nombre,v.tipo_comprobante,v.serie_comprobante,"
                    + "v.num_comprobante,v.fecha,v.impuesto,v.total,v.estado FROM venta v "
                    + "INNER JOIN persona p ON v.persona_id=p.id "
                    + "INNER JOIN usuario u ON v.usuario_id=u.id WHERE v.fecha>=? AND v.fecha<=?");
